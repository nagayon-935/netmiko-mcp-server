...
次に、デバイス設定ファイルをマウントしてコンテナを起動します。

```bash
docker run -d -p 10000:10000 -v $(pwd)/network_devices.toml:/app/config.toml --name netmiko-mcp netmiko-mcp-server --sse --port 10000
```
